...
const Shop = () => {
  ...
  useEffect(() => {
  axios.get("http://localhost:3000/api/furniture").then((response) =>setFilteredFurniture(response.data)).catch(error=>console.log(error));
  }, []);
  useEffect(() => {
    window.scroll(0, 0);
  }, [currentPage]);
  ...
  useEffect(() => {
    ...
    filteredFurniture.filter((furniture) => {
